...
//!
//! Defines configuration for SOCKS5 proxy connections, including
...
///
/// Supports username/password authentication and delegates common proxy
/// settings like timeouts and TLS to the base configuration via the
...
pub struct SOCKS5Config {
    ...
    // Target SOCKS5 proxy host.
    host: String,
    // Target SOCKS5 proxy port.
    port: u16,
...
impl SOCKS5Config {
    ...
    /// use roxie::config::SOCKS5Config;
    ///
    /// let config = SOCKS5Config::new("old.example.com", 1080)
    ...
    /// use roxie::config::SOCKS5Config;
    ///
    /// let config = SOCKS5Config::new("proxy.example.com", 1080).set_username("alice");
    ...
    /// use roxie::config::SOCKS5Config;
    ///
    /// let config = SOCKS5Config::new("proxy.example.com", 1080).set_password("secret");
...
impl Default for SOCKS5Config {
    /// Creates the default SOCKS5 proxy configuration (`localhost:1080`).
    ///
    /// # Examples
    ///
    /// ```
...
/// Wire SOCKS5Config into the shared base config trait.
///
/// This provides all the base config builder methods like `.set_handshake_timeout()`,
/// `.set_phase_timeout()`, `.set_auto_tls()`, etc. via `BaseProxyConfigBuilder`.
impl HasBaseProxyConfig for SOCKS5Config {
    ...
    /// ```
    /// use std::time::Duration;
    /// use roxie::config::{SOCKS5Config, HasBaseProxyConfig, BaseProxyConfigBuilder};
    ///
    /// let config = SOCKS5Config::new("proxy.example.com", 1080)
    ///     .set_handshake_timeout(Duration::from_secs(20));
    ///
    ...
    /// ```
    /// use std::time::Duration;
    /// use roxie::config::{SOCKS5Config, HasBaseProxyConfig};
    ///
    /// let mut config = SOCKS5Config::new("proxy.example.com", 1080);
...
mod tests {
    ...
    #[test]
    fn socks5_config_builder_chain() {
        let config = SOCKS5Config::new("proxy.example.com", 1080)
            .set_credentials("user", "pass")
            .set_handshake_timeout(Duration::from_secs(15))
            .set_tcp_nodelay(false)
    ...
    #[test]
    fn socks5_config_validation() {
        let valid = SOCKS5Config::new("proxy.com", 1080);
        assert!(valid.validate().is_ok());

        let invalid_port = SOCKS5Config::new("proxy.com", 0);
        assert!(invalid_port.validate().is_err());

        let invalid_host = SOCKS5Config::new("", 1080);
